...
| AC-1 | 정상 Normal | WHEN 학생이 수강 할 과목을 선택하면 THE  SHALL 커리큘럼과 강의평, 수강시간을 표시한다.  | 선택 시 과목을 3개 만든 뒤 강의평,커리큘럼,수강시간이 나오는지 확인 |
| AC-2 | 정상 Normal | WHEN 수강 할 과목을 개인 시간표에 넣게 되면 THE SHALL 요일과 시간을 표시하고 중복을 점검한다   | 시간이 겹치는 과목을 2개 만든 뒤 중복화면을 표시하는지 확인 |
| AC-3 | 정상 Normal | WHEN 커리큘럼 탭을 열면 THE SHALL 학년 별 커리큘럼을 표시한다. | 학과를 입력하면 그에 맞는 커리큘럼을 표시하는지 확인 |
| AC-4 | 실패 Failure | IF DB에 없는 강좌라면  THEN THE  SHALL "개설되지 않은 강좌입니다" 를 반환한다.  | DB에 없는 과목을 검색창에 검색해보고 오류화면을 표시하는지 확인 |
