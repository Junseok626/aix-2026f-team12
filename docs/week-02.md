...
A조의 실패는 모델 탓이 아니라, 조건을 제시하지 않은 탓입니다.
B조는 지시, 규약, 근거를 제시하여 가이드라인을 제시했지만 A조는 그러지 않았습니다.
이 때문에 A조의 경우 모델이 python을 언어로 선택하여 만들었고 본인 메모를 반환하는 현상은 B조만 종료 조건에서 제시하였기 때문입니다.


**4-3. B조가 준 자료 중 결과를 가장 크게 바꾼 것 하나를 꼽는다면 무엇입니까? 왜 그렇게 생각합니까?**

CONVENTIONS.md가 준 자료 중 결과를 가장 크게 바꾸었다. CONVENTIONS.md 파일에는 프로젝트에서 지켜야 할 계층 분리, 응답 형식, 입력 검증 등이 명시되어 있었고, 특히 모든 쿼리에 user_id 조건을 포함하여 본인 소유의 데이터만 조회하도록 규정되어 있었다. 따라서 B조의 결과는 이 규약을 반영하여 내 메모만 검색할 수 있었지만, A조는 다른 사람의 메모까지도 검색할 수 있게 되었다. 
